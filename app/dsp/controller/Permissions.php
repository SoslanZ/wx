...
class Permissions extends Controller
{
    ...
    {        
        ...
        if(!Session::has('admin')) {
            ...
            $this->redirect('dsp/common/login');
        ...
        $menus = Db::name('dsp_cate')->where('id',Session::get('admin_cate_id'))->value('permissions');
        //将得到的菜单id集成的字符串拆分成数组
        $menus = explode(',',$menus);

        if(!empty($string)) {
            //检查该带参数的url是否设置了权限
            $param_menu = Db::name('dsp_menu')->where($where)->where('parameter',$string)->find();
            ...
            if(!empty($param_menu)) {
                ...
                $menu = Db::name('dsp_menu')->where($where)->find();
                if(!empty($menu)) {
    ...
    protected function is_media()
    {
        return 3 == Session::get('admin_cate_id');
    }
    protected function is_caiwu()
    {
        return 4 == Session::get('admin_cate_id');
    }
