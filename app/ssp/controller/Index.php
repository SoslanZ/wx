...
class Index extends Permissions
{
    ...
    {
        $menu = Db::name('ssp_menu')->where(['is_display'=>1])->order('orders asc')->select();

        //删除不在当前角色权限里的菜单，实现隐藏
        $admin_cate = Session::get('admin_cate_id');

        $permissions = Db::name('ssp_cate')->where(['id'=>$admin_cate])->value('permissions');
        ...
        $cookie = Db::name('sspuser')->where('id',Session::get('admin'))->find();
