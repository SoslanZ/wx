...
use app\ssp\model\SspMenu as menuModel;
...
class Menu extends Permissions
{
		...
		if($id > 0) {
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post,['id'=>$id])) {
	            	...
	            	return $this->success('修改菜单信息成功','ssp/menu/index');
	            }
    		} else {
    			//非提交操作
    			$menu = $model->where('id',$id)->find();
    		...
    		if($this->request->isPost()) {
	            ...
	            if(false == $model->allowField(true)->save($post)) {
	            	...
	            	return $this->success('添加菜单成功','ssp/menu/index');
	            }
    		} else {
    			//非提交操作
    ...
    public function delete()
    {
    	if($this->request->isAjax()) {
    		$id = $this->request->has('id') ? $this->request->param('id', 0, 'intval') : 0;
            if(Db::name('ssp_menu')->where('pid',$id)->select() == null) {
                if(false == Db::name('ssp_menu')->where('id',$id)->delete()) {
                    return $this->error('删除失败');
                } else {
                    return $this->success('删除成功','ssp/menu/index');
                }
            } else {
    ...
    public function orders()
    {
        if($this->request->isPost()) {
            ...
            return $this->success('成功更新'.$i.'个数据','ssp/menu/index');
        }
    }
}
